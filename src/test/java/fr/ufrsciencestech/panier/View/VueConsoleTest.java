...
public class VueConsoleTest {
    ...
    public void testUpdate() throws PanierPleinException, PanierVideException {
        ...
        when(mockp2.getTaille()).thenReturn(2);   //comportement des doublures (stubbing)
        when(mockp0.getTaille()).thenReturn(0); 
        //tests d’interaction :
        verify(mockp2, times(0)).getTaille();    //getSize() doit avoir été appelé exactement 0 fois
        assertEquals(vuecmock.getTrace(), "Contenance initiale : " + 0);
        vuecmock.update(mockp2, null);
        verify(mockp2, times(1)).getTaille(); 
        assertEquals(vuecmock.getTrace(), "Nouvelle contenance : " + 2);
        
        vuecmock.update(mockp0, null);
        verify(mockp0, times(1)).getTaille(); 
